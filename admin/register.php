...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $stmt = $pdo->prepare("SELECT COUNT(*) FROM admin_credentials WHERE username = ? OR email = ?");
  ...
  if($count > 0) {
    ...
    $stmt = $pdo->prepare("INSERT INTO admin_credentials (username, email, password) VALUES (?, ?, ?)");
  ...
  <a href="admin-login.php"><span>Don't have an account? Register now!</span></a>
</div>
